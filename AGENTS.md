...
Original software: `bedstatus/` (status bar), `xbgcd/` (X background setter), `netris/` (tetris, needs ncurses + `-lbsd` on Linux), `misc/` (small utilities: `bidle`, `flash`, `slowcat`, `gurkx`, `xxed`, `pinentry-dmenu2`), `scripts/` (`dmenu_*`, `launch-*`, etc.).
